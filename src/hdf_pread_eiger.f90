...
program hdf_pread_eiger
        ...
        integer :: g_Nx  = 1024 ! Image slab size (in pixels)
        integer :: g_Ny  = 1024 ! Image slab size (in pixels)
        integer :: ch_Nx = 1024 ! Chunk size (in pixels)
        integer :: ch_Ny = 1024 ! Chunk size (in pixels)
        ...
        do idset = 1, ndsets
           ...
           do i = 1, nsetsdsetproc
              ...
              call h5sselect_hyperslab_f(filespace, H5S_SELECT_SET_F, &
                                         ...
                                         stride, block)
              ! Read the data
              call h5dread_f(d_id, H5T_IEEE_F64LE, ld, s_size, ierr,         &
        ...
        if (id .eq. 0) then
           ...
           stat_total = real((real(g_Nx*g_Ny)/1.e6) * nsets * 8) ! (MBytes)
